...
import {
  Search, ShoppingCart, Menu, X, ChevronDown, LogOut,
  Package, MapPin, Lock, Bot, Phone, MessageCircle,
  User as UserIcon, ArrowRight, School, GraduationCap, Building2,
} from 'lucide-react';
import { useAuthStore } from '@/store/auth.store';
import { useCartStore } from '@/store/cart.store';
import { cn, formatPrice } from '@/lib/utils';
...
// ─── Mega menu data (product catalogue — unchanged) ───────────────────────────
...
const MEGA_SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'AI MAGIC WORLD T1',     slug: 'sach-ai-magic-world-tap-1',          img: 'https://s4h.edu.vn/wp-content/uploads/2024/01/AI-MAGIC-WORLD-1.webp',            desc: 'Cấp 1 - Tập 1' },
      { name: 'AI MAGIC WORLD T2',     slug: 'sach-ai-magic-world-tap-2',          img: 'https://s4h.edu.vn/wp-content/uploads/2024/01/AI-MAGIC-WORLD-2.webp',            desc: 'Cấp 1 - Tập 2' },
      ...
      { name: 'AI SUPER ASSISTANT T1', slug: 'sach-ai-super-assistant-tap-1',      img: 'https://s4h.edu.vn/wp-content/uploads/2024/01/AI-Super-Assistant-1-1.webp',      desc: 'Cấp 2 - Tập 1' },
      { name: 'AI SUPER ASSISTANT T2', slug: 'sach-ai-super-assistant-tap-2',      img: 'https://s4h.edu.vn/wp-content/uploads/2024/01/AI-Super-Assistan-2-1.webp',       desc: 'Cấp 2 - Tập 2' },
      ...
      { name: 'AI SUPER DESIGNER T1',  slug: 'sach-ai-super-designer-tap-1',       img: 'https://s4h.edu.vn/wp-content/uploads/2024/01/AI-SUPER-DESIGNER-1.webp',         desc: 'Cấp 3 - Tập 1' },
      { name: 'AI SUPER DESIGNER T2',  slug: 'sach-ai-super-designer-tap-2',       img: 'https://s4h.edu.vn/wp-content/uploads/2024/01/AI-SUPER-DESIGNER-2.webp',         desc: 'Cấp 3 - Tập 2' },
    ],
  },
];

// ─── Solutions dropdown (new corporate nav) ───────────────────────────────────
...
const SOLUTIONS = [
  {
    ...
    desc: 'Chương trình AI & Robotics cho học sinh tiểu học đến THPT',
    badge: 'Phổ thông',
  ...
  },
  {
    ...
    icon: GraduationCap,
    href: '/ai-center',
    desc: 'AI Lab, AI Center và chương trình đào tạo cho sinh viên',
    badge: 'Đại học',
  ...
  },
  {
    ...
    desc: 'Robot dịch vụ, tự động hoá và triển khai giải pháp AI',
    badge: 'Enterprise',
...
const MAIN_NAV = [
  ...
  // 'Giải pháp' is rendered as a special dropdown — handled separately
  { label: 'Sản phẩm',  href: '/san-pham' },
  { label: 'Dự án',     href: '/dat-hang' },   // adjust href when /du-an route is ready
  { label: 'Tin tức',   href: '/tin-tuc' },
];
...
export default function Header() {
  ...
  const debounceRef     = useRef<ReturnType<typeof setTimeout>>();
  const megaCloseTimer  = useRef<ReturnType<typeof setTimeout>>();
  const solCloseTimer   = useRef<ReturnType<typeof setTimeout>>();
  ...
  // Mega menu (product catalogue)
  const openMega  = () => { clearTimeout(megaCloseTimer.current); setMegaOpen(true); };
  const closeMega = () => { megaCloseTimer.current = setTimeout(() => setMegaOpen(false), 100); };

  // Solutions dropdown
  const openSol  = () => { clearTimeout(solCloseTimer.current); setSolutionsOpen(true); };
  const closeSol = () => { solCloseTimer.current = setTimeout(() => setSolutionsOpen(false), 100); };
  ...
  const navLinkClass = (href: string) => cn(
    'px-3.5 h-11 flex items-center text-[13px] font-semibold transition-colors border-b-2 whitespace-nowrap',
    isActive(href)
  ...
  return (
    <>
      {/* ── Topbar ─────────────────────────────────────────────────────────── */}
      <div className="bg-slate-900 hidden sm:block">
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center h-9 text-xs text-slate-400">
          <span className="flex items-center gap-2">
      ...
      <header className={cn(
        ...
        scrolled ? 'shadow-[0_1px_20px_rgba(0,0,0,0.08)]' : 'shadow-[0_1px_0_rgba(0,0,0,0.06)]',
      )}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center h-16 gap-4">
          ...
          <Link href="/" className="flex-shrink-0 flex items-center group">
            ...
            />
            <span style={{ display: 'none' }} className="items-center gap-2">
              <span className="w-9 h-9 rounded-xl bg-slate-900 flex items-center justify-center text-white font-black text-base">U</span>
              <span className="font-black text-slate-900 text-xl tracking-tight">
                UBTECH<span className="text-sm ml-0.5 text-slate-400">VN</span>
              </span>
            </span>
          </Link>
          ...
          <div ref={searchRef} className="hidden md:flex flex-1 max-w-md mx-6 relative">
            <form onSubmit={handleSearch} className="w-full">
              <div className="relative">
                ...
                <input
                  ...
                  onFocus={() => suggests.length > 0 && setShowSuggest(true)}
                  placeholder="Tìm robot, kit AI, sách..."
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/20 focus:bg-white transition-all border border-transparent focus:border-slate-200"
                />
              </div>
            </form>
            ...
            {showSuggest && suggests.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-1.5 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50">
                {suggests.map(item => {
                  ...
                  return (
                    <Link key={item._id} href={`/san-pham/${item.slug}`}
                      ...
                      {img ? (
                        <img src={img.url} alt={item.name} className="w-10 h-10 object-contain rounded-lg bg-slate-50" />
                      ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400 font-medium">Xem chi tiết →</p>
                      </div>
                    </Link>
                  );
                })}
                <Link href={`/san-pham?search=${encodeURIComponent(search)}`}
                  onClick={() => { setShowSuggest(false); setSearch(''); }}
                  className="flex items-center justify-center gap-1.5 px-4 py-2.5 border-t border-slate-100 text-sm text-slate-600 font-semibold hover:bg-slate-50 transition-colors">
          ...
          <div className="flex items-center gap-1.5 ml-auto">
            ...
            <button onClick={() => setSearchOpen(!searchOpen)}
              className="md:hidden w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center transition-colors">
            ...
            <Link href="/gio-hang"
              ...
              {totalItems > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-slate-900 text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1">
                  {totalItems > 99 ? '99+' : totalItems}
                </span>
              )}
            </Link>
            ...
            {user ? (
              <div ref={userMenuRef} className="relative">
                ...
                {userOpen && (
                  <div className="absolute top-full right-0 mt-1.5 w-52 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-50">
              ...
              <div className="hidden sm:flex items-center gap-2">
                <Link href="/dang-nhap"
                  className="text-sm font-semibold text-slate-600 px-4 py-2 rounded-xl hover:bg-slate-100 transition-colors">
                  Đăng nhập
                </Link>
                <Link href="/dang-ky"
            ...
            {/* Mobile menu toggle */}
            <button onClick={() => setMobileOpen(!mobileOpen)}
              className="lg:hidden w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center transition-colors ml-1">
              {mobileOpen
          ...
          <div className="md:hidden px-4 pb-3">
            <form onSubmit={handleSearch}>
              <div className="relative">
                ...
                <input value={search} onChange={e => handleInput(e.target.value)}
                  placeholder="Tìm kiếm sản phẩm..." autoFocus
                  className="w-full pl-10 pr-4 py-2.5 bg-slate-50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/20" />
              </div>
            </form>
          </div>
        )}
        ...
        <nav className="hidden lg:block border-t border-slate-100">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center h-11">
            ...
            <div ref={solutionsRef} className="relative h-full flex items-center"
              onMouseEnter={openSol} onMouseLeave={closeSol}>
              <button className={cn(
                'flex items-center gap-1 px-3.5 h-11 text-[13px] font-semibold transition-colors border-b-2 whitespace-nowrap',
                solutionsOpen || ['/chuong-trinh-k12', '/ai-center', '/giai-phap-robot'].some(p => pathname.startsWith(p))
                  ? 'text-slate-900 border-slate-900'
                  : 'text-slate-500 hover:text-slate-900 border-transparent',
              )}>
                Giải pháp
              ...
              {solutionsOpen && (
                <div className="absolute top-full left-0 mt-0 z-50 w-[480px]"
                  onMouseEnter={openSol} onMouseLeave={closeSol}>
                  <div className="bg-white rounded-b-2xl rounded-tr-2xl shadow-2xl border border-t-0 border-slate-100 p-4">
                    ...
                    <div className="space-y-1">
                      {SOLUTIONS.map(sol => {
                        const Icon = sol.icon;
                        return (
                          <Link key={sol.key} href={sol.href}
                            onClick={() => setSolutionsOpen(false)}
                            className="flex items-start gap-3.5 px-3 py-3 rounded-xl hover:bg-slate-50 transition-colors group">
                            <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center flex-shrink-0 group-hover:bg-slate-200 transition-colors mt-0.5">
                              <Icon className="w-4.5 h-4.5 text-slate-600" />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-0.5">
                                <span className="text-[13px] font-semibold text-slate-800 group-hover:text-slate-900">
                                  {sol.label}
                                </span>
                                <span className={cn('text-[10px] font-semibold px-1.5 py-0.5 rounded-full', sol.badgeClass)}>
                                  {sol.badge}
                                </span>
                              </div>
                              <p className="text-[12px] text-slate-500 leading-relaxed">{sol.desc}</p>
                            </div>
                            <ArrowRight className="w-4 h-4 text-slate-300 group-hover:text-slate-500 group-hover:translate-x-0.5 transition-all mt-2.5 flex-shrink-0" />
                          </Link>
                        );
                      })}
                    </div>
                    <div className="mt-3 pt-3 border-t border-slate-100 px-1">
                      <Link href="/lien-he"
                        onClick={() => setSolutionsOpen(false)}
                        className="text-[12px] text-slate-500 hover:text-slate-900 transition-colors font-medium flex items-center gap-1">
                        Không chắc cần giải pháp nào? Liên hệ tư vấn →
                      </Link>
                    </div>
                  </div>
                </div>
            ...
            {/* Giải Pháp Robot — mega menu (product catalogue) */}
            <div ref={megaRef} className="relative h-full flex items-center"
              onMouseEnter={openMega} onMouseLeave={closeMega}>
              <button className={cn(
                'flex items-center gap-1 px-3.5 h-11 text-[13px] font-semibold transition-colors border-b-2 whitespace-nowrap',
                megaOpen ? 'text-slate-900 border-slate-900' : 'text-slate-500 hover:text-slate-900 border-transparent',
              )}>
                Sản phẩm
              ...
              {megaOpen && (
                <div className="absolute top-full left-0 mt-0 z-50" style={{ width: '860px' }}
                  onMouseEnter={openMega} onMouseLeave={closeMega}>
                  <div className="bg-white rounded-b-2xl rounded-tr-2xl shadow-2xl border border-t-0 border-slate-100 overflow-hidden flex" style={{ maxHeight: '500px' }}>
                    {/* Sidebar */}
                    <div className="w-44 flex-shrink-0 bg-slate-900 py-3">
                      {MEGA_SECTIONS.map((sec, idx) => (
                        <button key={sec.title} onMouseEnter={() => setActiveSection(idx)}
                          className={cn(
                            'w-full text-left px-4 py-3 text-[11px] font-bold uppercase tracking-[0.1em] transition-all flex items-center justify-between',
                    ...
                    <div className="flex-1 p-5 overflow-y-auto">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <Link href="/san-pham" onClick={() => setMegaOpen(false)}
                          className="text-[11px] text-slate-600 font-semibold hover:text-slate-900 transition-colors">
                          Xem tất cả →
                        </Link>
                      </div>
                      <div className="grid grid-cols-3 gap-3">
                        {MEGA_SECTIONS[activeSection].items.map(item => (
                          <Link key={item.slug} href={`/san-pham/${item.slug}`} onClick={() => setMegaOpen(false)}
                            className="group flex flex-col items-center gap-2 p-3 rounded-xl hover:bg-slate-50 transition-all">
                            <div className="w-20 h-20 flex items-center justify-center rounded-xl bg-slate-50 group-hover:bg-white transition-colors overflow-hidden border border-slate-100">
                              <img src={item.img} alt={item.name}
                                className="w-full h-full object-contain"
                                onError={e => { (e.target as HTMLImageElement).style.opacity = '0.2'; }} />
                            </div>
                            <div className="text-center">
                              <p className="text-[11px] font-bold text-slate-700 group-hover:text-slate-900 leading-tight">{item.name}</p>
            ...
            {/* Liên hệ — prominent CTA */}
            <div className="ml-auto flex items-center gap-3">
              <a href={`tel:${PHONE}`}
                className="hidden xl:flex items-center gap-1.5 text-[12px] font-semibold text-slate-500 hover:text-slate-800 transition-colors">
        ...
        {/* ── Mobile menu overlay ────────────────────────────────────────────── */}
        {mobileOpen && (
          <div className="lg:hidden fixed inset-0 top-[calc(4rem+2.25rem)] z-40 bg-white overflow-y-auto">
            <nav className="px-4 pt-4 pb-8 space-y-1">

              {/* Trang chủ */}
              <Link href="/" onClick={() => setMobileOpen(false)}
                className={cn(
                  'block px-4 py-3 rounded-xl text-[14px] font-semibold transition-colors',
              ...
              {/* Giải pháp accordion */}
              <div>
                <button onClick={() => setMobileExpanded(mobileExpanded === 'giai-phap' ? null : 'giai-phap')}
                  className="flex items-center justify-between w-full px-4 py-3 rounded-xl text-[14px] font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
                  Giải pháp
                  <ChevronDown className={cn('w-4 h-4 text-slate-400 transition-transform duration-200', mobileExpanded === 'giai-phap' && 'rotate-180')} />
                </button>
                {mobileExpanded === 'giai-phap' && (
                  <div className="mt-1 ml-2 space-y-1 border-l-2 border-slate-100 pl-3">
                    {SOLUTIONS.map(sol => {
                      const Icon = sol.icon;
                      return (
                        <Link key={sol.key} href={sol.href} onClick={() => setMobileOpen(false)}
                          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors">
                          <Icon className="w-4 h-4 text-slate-400 flex-shrink-0" />
                          <span className="font-medium">{sol.label}</span>
                          <span className={cn('ml-auto text-[10px] font-semibold px-1.5 py-0.5 rounded-full', sol.badgeClass)}>
                            {sol.badge}
                          </span>
                        </Link>
                      );
              ...
              {/* Sản phẩm accordion (mega) */}
              <div>
                <button onClick={() => setMobileExpanded(mobileExpanded === 'san-pham' ? null : 'san-pham')}
                  className="flex items-center justify-between w-full px-4 py-3 rounded-xl text-[14px] font-semibold text-slate-700 hover:bg-slate-50 transition-colors">
                  Sản phẩm
                  <ChevronDown className={cn('w-4 h-4 text-slate-400 transition-transform duration-200', mobileExpanded === 'san-pham' && 'rotate-180')} />
                </button>
                {mobileExpanded === 'san-pham' && (
                  <div className="mt-1 ml-2 space-y-3 border-l-2 border-slate-100 pl-3">
                    {MEGA_SECTIONS.map(sec => (
                      <div key={sec.title}>
                        <p className="px-3 py-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider">{sec.title}</p>
                        {sec.items.map(item => (
                          <Link key={item.slug} href={`/san-pham/${item.slug}`} onClick={() => setMobileOpen(false)}
                            className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-[13px] text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors">
                    ...
                    <Link href="/san-pham" onClick={() => setMobileOpen(false)}
                      className="flex items-center gap-1.5 px-3 py-2 text-[13px] font-semibold text-slate-500 hover:text-slate-900 transition-colors">
              ...
              <div className="pt-2">
                <Link href="/lien-he" onClick={() => setMobileOpen(false)}
                  className="flex items-center justify-center gap-2 w-full px-4 py-3.5 bg-slate-900 text-white rounded-xl text-[14px] font-semibold hover:bg-slate-700 transition-colors">
                  Liên hệ tư vấn
              ...
              <div className="pt-1">
                <a href={`tel:${PHONE}`}
                  className="flex items-center justify-center gap-2 w-full px-4 py-3 border border-slate-200 rounded-xl text-[13px] font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
              ...
              {!user && (
                <div className="flex gap-2 pt-2 border-t border-slate-100">
                  <Link href="/dang-nhap" onClick={() => setMobileOpen(false)}
                    className="flex-1 text-center text-[13px] font-semibold text-slate-700 px-4 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 transition-colors">
                    Đăng nhập
                  </Link>
                  <Link href="/dang-ky" onClick={() => setMobileOpen(false)}
                    className="flex-1 text-center text-[13px] font-semibold bg-slate-900 text-white px-4 py-2.5 rounded-xl hover:bg-slate-700 transition-colors">
                    Đăng ký
                  </Link>
                </div>
              )}
